...
# from .models import Article, Category, SubCategory, Tag
...
class IndexView(generic.ListView):
    ...
    def get_queryset(self):
        ...
        # 日期归档
        year = self.kwargs.get('year', 0) #从url里面获取关键字, 可以参照base.html里面的第96行, 可以把url里面的关键字看成事字典
        #<a href='{% url 'blog:date' date|date:'Y' date|date:'m' %}'>
        ...
        print(slug)
        if self.big_slug:
            big = get_object_or_404(Category, description=self.big_slug)
            queryset = queryset.filter(category__blog_type=big)

            if slug:
                slu = get_object_or_404(SubCategory, name=slug)
                queryset = queryset.filter(category=slu)

        if year and month:
            queryset = get_list_or_404(queryset, pub_date__year=year, pub_date__month=month)

        if tag:
            tags = get_object_or_404(Tag, name=tag)
            # self.big_slug = Category.objects.filter(category__article__tags=tags)
...
class MySearchView(SearchView):
    ...
    paginate_orphans = getattr(settings, 'BASE_ORPHANS', 0)
    # 搜索结果以浏览量排序
    queryset = SearchQuerySet().order_by('-visitors')

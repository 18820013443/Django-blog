...
from account.models import Ouser
...
# 递归生成HTML字符串，展示评论区内容
def generate_comment_html(sub_comment_dic, category, request, s=2):
    html = "<ul class='children'>"
    # 对传入的字典进行循环操作
    for k, v_dic in sub_comment_dic.items():
        user = Ouser.objects.filter(email = k.author.email)
        if user:
            profile_link = 'media/%s'%(user[0].avatar)
        else:
            profile_link = 'media/avatar/default.png'
        content = k.content_to_markdown()
        html += '''
<li class="comment odd alt depth-{0}" id="comment-{1}"><div class="c-avatar"><img alt='' data-original='http://{7}/{8}' class='avatar avatar-54 photo' height='54' width='54' /><div class="c-main" id="div-comment-{1}">{2}<div class="c-meta"><span class="c-author"><a href='http://fsfs' rel='external nofollow' class='url'>{3}</a></span>{4}<a rel='nofollow' class='comment-reply-link' href='{6}?replytocom={1}#respond' onclick='return addComment.moveForm( "div-comment-{1}", "{1}", "respond", "{5}" )' aria-label='回复给{3}'>回复</a></div></div></div>'''.format(s,k.id,content,k.author.nickname,k.create_date.strftime('%Y-%m-%d %H:%M:%S'),category,request.path, request.get_host(), profile_link)

        # 有可能v_dic中依然有元素, 递归继续加
        if v_dic:
            s += 1
            html += generate_comment_html(v_dic, category, request, s)
        html += "</li>"
    # 循环完成最后返回html
    html += "</ul>"
    return html
...
# 生成层级评论
@register.simple_tag
def build_comment_tree(category, request, entry=0):
    if category == 'about':
    ...
    for k, v in comment_dic.items():
        ...
        content = k.content_to_markdown()
        # 第一层html
        html += '''<li class="comment even thread-even depth-1" id="comment-{0}"><div class="c-avatar"><img alt='' data-original='http://{6}/{7}' class='avatar avatar-54 photo' height='54' width='54' /><div class="c-main" id="div-comment-{0}">{1}<div class="c-meta"><span class="c-author">{2}</span>{3}<a rel='nofollow' class='comment-reply-link' href='{5}?replytocom={0}#respond' onclick='return addComment.moveForm( "div-comment-{0}", "{0}", "respond", "{4}" )' aria-label='回复给{2}'>回复</a></div></div></div>'''.format(k.id,content,k.author.nickname, k.create_date.strftime('%Y-%m-%d %H:%M:%S'), category, request.path, request.get_host(),profile_link)
        # 通过递归把他的儿子加上
        html += generate_comment_html(v, category, request)
    # 最后把ul关上
    html += " </ol>"
    # 关掉转义
